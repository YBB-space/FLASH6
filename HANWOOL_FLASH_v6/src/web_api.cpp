...
void buildJson(char* out, size_t outLen, const SampleSnap& s) {
  snprintf(out, outLen,
           "{\"t\":%.3f,\"p\":%.3f,\"iv\":%.3f,\"bp\":%u,\"ut\":%lu,\"lt\":%u,\"ct\":%u,\"hz\":%u,"
           "\"s\":%u,\"ic\":%u,\"r\":%u,\"gs\":%u,"
           "\"st\":%u,\"cd\":%u,\"td\":%ld,\"uw\":%u,\"ab\":%u,\"ar\":%u,\"m\":%u,"
           "\"rs\":%u,\"rf\":%u,\"rm\":%u,\"ss\":%u,\"sm\":%u,"
           "\"fw_program\":\"%s\",\"fw_ver\":\"%s\",\"fw_build\":\"%s\","
           "\"fw_ver_build\":\"%s\",\"fw_board\":\"%s\",\"fw_protocol\":\"%s\"}",
           s.t, s.p, s.iv, (unsigned)s.bp, (unsigned long)s.ut, (unsigned)s.lt, (unsigned)s.ct, (unsigned)s.hz,
           (unsigned)s.s, (unsigned)s.ic, (unsigned)s.r, (unsigned)s.gs,
           (unsigned)s.st, (unsigned)s.cd, (long)s.td, (unsigned)s.uw, (unsigned)s.ab, (unsigned)s.ar, (unsigned)s.m,
...
static void handleSequenceEnd(AsyncWebServerRequest* request) {
  if (isLocked()) { sendLockedHttp(request); return; }
  const uint32_t now = millis();
  portENTER_CRITICAL(&stateMux);
  endSequenceNow(now);
  portEXIT_CRITICAL(&stateMux);
  request->send(200, "text/plain", "SEQUENCE_ENDED");
}
...
static void handleEasterBgm(AsyncWebServerRequest* request) {
  requestTetrisBgm();
  request->send(200, "text/plain", "EASTER_BGM_OK");
}
...
void setupWebServer() {
  WiFi.mode(WIFI_AP);
  WiFi.setSleep(false);
  WiFi.softAP("ALTIS INTELLIGENT V1", "12345678");
  ...
  server.on("/dashboard.html",  HTTP_GET, dashboard);
